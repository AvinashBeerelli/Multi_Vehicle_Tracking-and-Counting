...
def main(_argv):
    ...
    up_count = 0
    down_count = 0
    total_counter_per_min = 0
    up_count_per_min = 0
    down_count_per_min = 0

    class_counter = Counter()  # store counts of each detected class
    class_up_counter = Counter()
    class_down_counter = Counter()
    class_up_counter_per_min = Counter()
    class_down_counter_per_min = Counter()
    ...
    while True:
		...
		# Vehicle Counting Code
            ...
            if intersect(midpoint, previous_midpoint, line[0], line[1]) and track.track_id not in already_counted:
                    ...
                    # draw red line
                    cv2.line(frame, line[0], line[1], (225, 0, 0), 2)
                    ...
                    intersect_info.append([track_cls, origin_midpoint, angle])
					
                    if angle > 0:
                        up_count += 1
                        class_up_counter[track_cls] += 1
                    if angle < 0:
                        down_count += 1
                        class_down_counter[track_cls] += 1
            ...
            cv2.putText(frame, "Total: {} ({} upward, {} downward)".format(str(total_counter), str(up_count),str(down_count)), (int(0.05 * frame.shape[1]), int(0.8 * frame.shape[0])), 0,1.5e-3 * frame.shape[0], (0, 255, 255), 2)
            ...
            for cls in class_counter:
                ...
                cv2.putText(frame, str(cls) + " " + str(class_count), (int(0.05 * frame.shape[1]), int(y)), 0, 1.5e-3 * frame.shape[0], (0, 255, 0), 2)
                y += 0.05 * frame.shape[0]
            ...
            if current_minute == 0 and len(count_dict) > 1:
                ...
                # write counts to file for every set interval of the minute
                write_interval = 1
                if current_minute % write_interval == 0:  # write to file once only every write_interval minutes
                    if current_minute not in count_dict:
                        ...
                        total_filename = 'Total counts for {}.txt'.format(current_date)
                        ...
                        total_counter_per_min = total_counter - total_counter_per_min
                        up_count_per_min = up_count - up_count_per_min
                        down_count_per_min = down_count - down_count_per_min
                        class_up_counter_per_min = class_up_counter - class_up_counter_per_min
                        class_down_counter_per_min = class_down_counter - class_down_counter_per_min
						
                        print("total_counter_per_min: ", total_counter_per_min )
                        print('{} writing...'.format(rounded_now))
                        print('Writing current total count ({}) and directional counts to file.'.format(total_counter))
                        # total_count_file.write('{}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}\n'.format(str(rounded_now),
                        #                                                    str(total_counter), up_count, down_count, total_counter_per_min,up_count_per_min,down_count_per_min, class_counter, class_up_counter, class_down_counter, class_up_counter_per_min, class_down_counter_per_min ))

                        total_count_file.write('{}, {}, {}, {}, {}\n'.format(str(rounded_now), up_count_per_min, class_up_counter_per_min, down_count_per_min, class_down_counter_per_min))
